...
- `bin\nticonx.exe` is the Icon virtual machine, which executes "icode".
  - See `iconx.*` below for a convient way to invoke this indirectly.
...
- `icont_nosmudge.cmd` invokes `icont` with a directive **not** to ensure that the portable directry is on the PATH.
...
- [`examples\smoke_test.cmd`](./examples/smoke_test.cmd) demonstrates use of the following files:
  ...
  - `examples\example_shebang.cmd` and `examples\#!icon.cmd` leverage `icon.cmd` to execute a self-translating script.
    - See smoke test 8 in `smoke_test.cmd`.
    - See also "Shebang - scripting with Icon" for details.
  - `examples\example_findstr.cmd` and the built-in Microsoft findstr to execute a self-translating script.
    - See smoke test 8 in `smoke_test.cmd`.
    - See also "Shebang - scripting with Icon" for details.
